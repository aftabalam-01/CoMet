...
export const createTransport = asyncHandler(async (req: Request, res: Response) => {
  const {
    name,
    email,
    phone,
...
export const getTransport = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const getTransports = asyncHandler(async (req: Request, res: Response) => {
  try {
    const {
      name,
      totalBoxes,
    ...
    const [transports, total] = await prisma.$transaction([
      prisma.transport.findMany({ where: filters, orderBy, skip, take: limitNum }),
...
export const updateTransport = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Transport Updated Successfully", updatedTransport });
  } catch (error: any) {
    throw new Error(error);
  }
});
...
export const deleteTransport = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Transport Deleted Successfully", deletedTransport });
  } catch (error: any) {
    throw new Error(error);
  }
});
...
export const deleteBulkTransport = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const deleteBulk = await prisma.transport.deleteMany({
      where: {
        id: {
          in: ids,
        },
